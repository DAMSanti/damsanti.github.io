...
class MiniGame7:
    ...
    def crea_problemas(self):
        self.numeros_generados = [self.genera_numeros() for _ in range(40)]
        ...
        for grupo in self.grupos_numeros:
            ...
            if guardar_resultado:
                ...
                self.resultados.append((num1, operador, num2)) 
            guardar_resultado = not guardar_resultado
        self.grupos_resultado = [self.resultados[i:i + 2] for i in range(0, len(self.resultados), 2)]
    ...
    def combinar_y_ejecutar_codigo_java(self, texto_usuario1, problema_index):
        ...
        problema = self.grupos_resultado[problema_index]
        problema1, problema2 = problema
        num11, operador1, num12, resultado = problema1
        num21, operador2, num22 = problema2
        codigo_java = f"""
public class MiClase {{
    public static void main(String[] args) {{
        ...
        boolean res = (num1 {operador1} num2) {texto_usuario1} (num3 {operador2} num4);
...
"""
    ...
    def draw_problemas(self, problema_index):
        if 0 <= problema_index < len(self.grupos_resultado):
            problema = self.grupos_resultado[problema_index]
            problema1, problema2 = problema
            num11, operador1, num12, resultado = problema1
            num21, operador2, num22 = problema2
            ...
            MiniGames.pinta(x_pos, y_pos, f"boolean resultado", 1)
            x_pos =  panel_x + panel_width * 0.45
            MiniGames.pinta(x_pos, y_pos, f"({num11}{operador1}{num12})", 1)
            x_pos = panel_x + panel_width * 0.65
            MiniGames.pinta(x_pos, y_pos, f"({num21}{operador2}{num22});", 1)
